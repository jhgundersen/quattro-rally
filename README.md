...
The eighth stage is a wide, banked tri-oval with a long backstretch, pit garages, grandstands and an infield Lake Lloyd. Its turns reach 31° of banking, inspired by [Daytona's high banks](https://www.daytonainternationalspeedway.com/about-daytona-international-speedway/), with gentler straights and smooth transitions. The road rises toward the outside wall; cars align to its normal and gravity pulls them toward the lower lane. Road geometry, markings, tyre trails and car contact share the same height model.

The oval is drawn square and then turned 45° on the board. A steeply banked turn that tilts away from the fixed isometric camera is almost edge-on and disappears; turned across the view, both banks read clearly and the tri-oval fills the widescreen frame. Outside the wall a grass bank falls to a low retaining plinth, and inside a verge runs down to the infield, so the banking stands on the ground as earthwork rather than hovering as a shelf. The full course remains visible from the fixed isometric camera.

## Online multiplayer

Choose a driver portrait, then **Create a room**, copy its invitation link, and send it to friends. Up to four people can join without accounts. Twelve portraits are available and you can change yours in the lobby; the car livery belongs to the seat. The lobby has a chat for the whole room. The host picks any track; everyone marks ready, then the host starts the race. Empty seats use AI. Each human has the same car performance. The chequered flag comes out when three cars are home, so nobody waits on the last car. After the results, the host returns everyone to the same lobby for another race.

Pausing or switching tabs hands your car to AI while the race continues. Connections recover automatically; reloading and joining again in the same tab restores your seat. Multiplayer results do not replace solo best times. Rooms are temporary and end when the backend restarts.

For local multiplayer, also run `npm run dev:server`. Production uses a small Node WebSocket service on the existing server, with authoritative shared physics and browser prediction. The regular build/deployment includes its bundle. See [multiplayer operations](deploy/multiplayer/README.md) for installation, limits and health checks.
