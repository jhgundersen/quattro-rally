...
export function createDrawbridges(group,course,track){
 ...
 for(const bridge of track.drawbridges||[]){
  ...
  const leaves=[],gates=[],lights=[];
  for(const sign of [-1,1]){
   box(track.width+1.7,.8,1,'#a79d84',0,-.4,sign*(half+.5),root);
   const leaf=new THREE.Group();leaf.name='drawbridge-leaf';leaf.userData={bridge,sign};leaf.position.set(0,-.1,sign*half);root.add(leaf);
   box(track.width,.22,half,'#737d86',0,0,-sign*half/2,leaf);
   box(.2,.025,half-.2,'#e5c94e',0,.125,-sign*half/2,leaf);
   ...
   const gate=new THREE.Group();gate.position.set(-track.width/2,.8,sign*(half+1.1));root.add(gate);
   for(let i=0;i<8;i++)box(track.width/8,.16,.13,i%2?'#eadfc5':'#b74336',(i+.5)*track.width/8,0,0,gate);
   gates.push(gate);
  }
  ...
  bridges.push({bridge,leaves,gates,lights});
 }
 function update(){for(const {bridge,leaves,gates,lights} of bridges){const state=drawbridgeState(bridge,course.traffic.time);for(const {leaf,sign} of leaves)leaf.rotation.x=sign*state.angle;for(const gate of gates)gate.rotation.z=state.blocked?0:Math.PI/2;for(const light of lights)light.material.color.set(state.blocked?'#ed493c':state.warning?'#eac04b':'#82b65b');}}
