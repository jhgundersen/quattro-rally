...
import { createStavanger } from './stavanger.js';
import {createDrawbridges} from './drawbridge-world.js';
import {onDrawbridge} from './drawbridges.js';
import {createStuntPark} from './stunt-world.js';
import {inLoop} from './stunt-motion.js';
...
export function createWorld(scene,track){
 ...
 function ribbon(a,b,color,y){const vertices=[],indices=[];for(let i=0;i<=N;i++)for(const offset of [a,b]){const p=at(i/N,offset);vertices.push(p.x,p.y+y,p.z);}for(let i=0;i<N;i++){if(inLoop(track,(i+.5)/N)||onDrawbridge(track,(i+.5)/N,course.length))continue;const a=i*2;indices.push(a,a+1,a+2,a+1,a+3,a+2);}const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(vertices,3));g.setIndex(indices);g.computeVertexNormals();mesh(g,color);}
 const plate=track.banking?[140,128]:track.id==='stavanger'?[154,96]:track.rails?[148,106]:[126,94];box(plate[0],1,plate[1],track.ground,0,-.65,0);ribbon(-track.width/2,track.width/2,track.road,.01);ribbon(-2.5,-1.4,track.rut,.025);ribbon(1.4,2.5,track.rut,.025);
 const barrierCount=Math.ceil(course.length/2.5);
 for(let i=0;i<barrierCount;i++)for(const side of [-1,1]){
   const t=i/barrierCount;if(inLoop(track,t)||onDrawbridge(track,t,course.length,1))continue;const p=at(t,side*(track.width/2+.85)),d=curve.getTangentAt(t);
   if(track.crossings?.some(c=>Math.hypot(p.x-c.x,p.z-c.z)<c.radius)||track.rails?.some(r=>Math.abs(p.z-r.z)<2.4))continue;
   const wall=(track.id==='lemans'||track.stuntPark)?.2:track.id==='stavanger'?.45:.65;
   const barrier=box(.65,wall,2.2,track.banking?track.edge:i%4<2?track.edge:track.id==='lemans'?'#386a9c':track.id==='stavanger'?'#78828b':track.stuntPark?'#bd4937':'#a9513d',p.x,p.y+wall/2,p.z);
   barrier.rotation.y=Math.atan2(d.x,d.z);if(track.stuntPark)barrier.quaternion.premultiply(new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0,1,0),course.roadFrame(p.x,p.z,t).normal));
 }
 ...
 for(let i=0;i<(track.banking?0:Math.ceil(course.length/28));i++){
   const t=(.09+i*28/course.length)%1;if(inLoop(track,t)||onDrawbridge(track,t,course.length,1))continue;const p=at(t),d=curve.getTangentAt(t);
   const arrow=mesh(new THREE.ShapeGeometry(arrowShape),track.edge,p.x,p.y+.075,p.z);
   arrow.rotation.set(-Math.PI/2,0,Math.atan2(-d.x,-d.z));arrow.castShadow=false;if(track.hills){const normal=course.roadFrame(p.x,p.z).normal;arrow.quaternion.premultiply(new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0,1,0),normal));}
 }
 for(let i=0;i<(['garage','lemans','daytona','stavanger','stunts'].includes(track.id)?0:130);i++){const p=at(random(),(random()-.5)*track.width);box(.1+random()*.15,.04,.13,track.rut,p.x,.045,p.z);}
 ...
 const line=new THREE.Group();line.position.copy(at(.035));const d=curve.getTangentAt(.035);line.rotation.set(0,Math.atan2(d.x,d.z),course.bankAngle(.035),'YXZ');if(track.hills){const normal=course.roadFrame(line.position.x,line.position.z).normal;line.quaternion.premultiply(new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0,1,0),normal));}group.add(line);
 ...
 const bannerY=course.terrainHeight(bannerX,bannerZ);sign(track.name.toUpperCase(),20,2,bannerX,bannerY+4.5,bannerZ);for(const x of [-9,9])box(.3,5,.3,'#303b31',bannerX+x,bannerY+2.2,bannerZ);framingPoints.push(new THREE.Vector3(bannerX,bannerY+6,bannerZ));
 ...
 for(let i=0;i<(['garage','daytona','stavanger','railyard','stunts'].includes(track.id)?0:65);i++){const x=(random()-.5)*104,z=(random()-.5)*73;if(track.id==='lemans'&&x<-30&&z>-8&&z<20)continue;if(nearest(x,z).distance<track.width/2+3||Math.hypot(x-landX,z-landZ)<8||Math.abs(x-bannerX)<12&&Math.abs(z-bannerZ)<4)continue;
 ...
 if(track.id==='stavanger')framingPoints.push(...createStavanger(group,course,track));
 const railWorld=track.rails?createRailyard(group,course,track):null;if(railWorld)framingPoints.push(...railWorld.framingPoints);
 const patches=track.patches.map(p=>{const pos=at(p.t,p.lane),d=curve.getTangentAt(p.t);const m=mesh(new THREE.CylinderGeometry(1,1,.055,40),{water:'#538e9f',mud:'#604735',ice:'#91c4d9',sand:'#ecc58c',oil:'#393346',cobble:'#9a978d'}[p.type],pos.x,.09,pos.z);m.scale.set(p.width/2,1,p.length/2);m.rotation.y=Math.atan2(d.x,d.z);
 ...
 if(track.stuntPark)framingPoints.push(...createStuntPark(group,course,track).framingPoints);
 const bridgeWorld=track.drawbridges?createDrawbridges(group,course,track):null;if(bridgeWorld)framingPoints.push(...bridgeWorld.framingPoints);
 ...
 return {...course,framingPoints,patches,obstacles,updateTraffic(){railWorld?.updateTraffic();bridgeWorld?.update();},dispose(){scene.remove(group);const geometries=new Set(),mats=new Set();group.traverse(o=>{if(o.geometry)geometries.add(o.geometry);if(o.material)mats.add(o.material);});geometries.forEach(g=>g.dispose());mats.forEach(m=>{m.map?.dispose();m.dispose();});}};
}
