...
export function createWorld(scene,track){
 ...
 for(let i=0;i<barrierCount;i++)for(const side of [-1,1]){
   ...
   const wall=track.id==='lemans'?.2:track.id==='stavanger'?.45:.65;
   const barrier=box(.65,wall,2.2,track.banking?track.edge:i%4<2?track.edge:track.id==='lemans'?'#386a9c':track.id==='stavanger'?'#78828b':'#a9513d',p.x,p.y+wall/2,p.z);
 ...
 for(let i=0;i<(['garage','lemans','daytona','stavanger'].includes(track.id)?0:130);i++){const p=at(random(),(random()-.5)*track.width);box(.1+random()*.15,.04,.13,track.rut,p.x,.045,p.z);}
 ...
 for(let i=0;i<(['garage','daytona','stavanger','railyard'].includes(track.id)?0:65);i++){const x=(random()-.5)*104,z=(random()-.5)*73;if(track.id==='lemans'&&x<-30&&z>-8&&z<20)continue;if(nearest(x,z).distance<track.width/2+3||Math.hypot(x-landX,z-landZ)<8||Math.abs(x-bannerX)<12&&Math.abs(z-bannerZ)<4)continue;
