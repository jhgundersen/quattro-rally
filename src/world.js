...
export function createWorld(scene,track){
 ...
 const plate=track.banking?[140,128]:track.id==='stavanger'?[154,96]:[126,94];box(plate[0],1,plate[1],track.ground,0,-.65,0);ribbon(-track.width/2,track.width/2,track.road,.01);ribbon(-2.5,-1.4,track.rut,.025);ribbon(1.4,2.5,track.rut,.025);
 ...
 for(let i=0;i<(['garage','daytona','stavanger'].includes(track.id)?0:65);i++){const x=(random()-.5)*104,z=(random()-.5)*73;if(track.id==='lemans'&&x<-30&&z>-8&&z<20)continue;if(nearest(x,z).distance<track.width/2+3||Math.hypot(x-landX,z-landZ)<8||Math.abs(x-bannerX)<12&&Math.abs(z-bannerZ)<4)continue;
