...
function reset(){raceTime=0;keys.clear();cars.forEach((c,i)=>{const t=startT-.022-Math.floor(i/2)*.026,p=at(t,i%2?2:-2),d=world.curve.getTangentAt((t+1)%1);Object.assign(c,{x:p.x,z:p.z,vx:0,vz:0,angle:Math.atan2(d.x,d.z),t:(t+1)%1,progress:t-startT,nitro:100,air:0,vy:0,jumpCooldown:0,finished:false,finishTime:0,surface:'gravel'});});dust.forEach(p=>{p.life=0;p.m.material.opacity=0;});$('race-message').textContent='';syncModels();updateHUD();}
function start(){reset();document.querySelectorAll('.track-card').forEach(b=>b.disabled=true);state='countdown';countdown=3.4;$('overlay').classList.add('hidden');$('status').textContent='ENGINES READY';$('pause').textContent='Ⅱ';}
...
window.quattro={get track(){return track.id;},get state(){return state;},get raceTime(){return raceTime;},get cars(){return cars.map(({x,z,angle,progress,nitro,air,finished,surface})=>({x,z,angle,progress,nitro,air,finished,surface}));},get drawCalls(){return renderer.info.render.calls;}};
