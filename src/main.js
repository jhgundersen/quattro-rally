...
function reset(){$('tournament-results').hidden=true;$('result-times').hidden=false;$('results').classList.remove('is-championship');cockpit.reset();world.setTime(0);$('results').classList.add('hidden');$('results').classList.remove('is-triumph');quipDriver=-1;$('race-message').textContent='';raceTime=0;keys.clear();cars.forEach((c,i)=>{// Half a car length of stagger inside each row, so the standings read P1..P4.
 const slot=gridSlot(i),t=startT-.022-Math.floor(slot/2)*.026-(slot%2)*.003,p=at(t,slot%2?2:-2),d=world.curve.getTangentAt((t+1)%1);Object.assign(c,{x:p.x,z:p.z,vx:0,vz:0,angle:Math.atan2(d.x,d.z),t:(t+1)%1,progress:t-startT,nitro:100,air:0,vy:0,jumpCooldown:0,finished:false,finishTime:0,stunt:null,stuntHeight:0,bridgeGround:0,bridgeSlope:0,bridgeHeading:0,surface:track.surface||'gravel'});});dust.clear();trails.forEach(t=>t.clear());$('race-message').textContent='';syncModels();updateHUD();}
...
function receiveSnapshot(s){
 ...
 if(s.phase==='racing'&&!s.cars[player].finished){
  // Online cars are non-solid. Replay only our inputs against the course;
  // delayed opponent snapshots must never push predicted positions around.
...
function syncModels(){
 for(const c of cars){
  ...
  if(c.stunt){const pose=loopCarPose(world,track,c.stunt);c.g.position.copy(pose.position);c.g.quaternion.copy(pose.quaternion);if(c.stunt.facing<0)c.g.rotateY(Math.PI);}
   ...
   if(c.bridgeGround)frame.normal.set(-Math.sin(c.bridgeHeading)*c.bridgeSlope,1,-Math.cos(c.bridgeHeading)*c.bridgeSlope).normalize();
   else if(track.drawbridges&&c.air>.1){const speed=Math.max(8,Math.hypot(c.vx,c.vz));frame.normal.set(-Math.sin(c.angle)*c.vy/speed,1,-Math.cos(c.angle)*c.vy/speed).normalize();}
