 ...
 return function update(car,time){
  if(previous){
   ...
   if(dt>0&&dt<=.25&&Math.hypot(dx,dz)<6){
    ...
    const distance=car.stunt?car.stunt.speed*dt*(car.stunt.direction||1)*(car.stunt.facing||1):dx*Math.sin(heading)+dz*Math.cos(heading);
    spin=(spin+distance/.44)%TAU;
    const target=!car.stunt&&Math.abs(distance)>.015?Math.max(-.48,Math.min(.48,Math.atan(2.23*yaw/distance))):0;
